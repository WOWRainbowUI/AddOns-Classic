...
function ScenarioBlocksFrame_OnLoad(self)
	...
	--ScenarioWidgetContainerBlock.module = SCENARIO_TRACKER_MODULE;  -- MSA
	--ScenarioWidgetContainerBlock:RegisterForWidgetSet(SCENARIO_TRACKER_WIDGET_SET, WidgetsLayout);  -- MSA
	...
	--self:RegisterEvent("PROVING_GROUNDS_SCORE_UPDATE");  -- MSA
...
function ScenarioStage_CustomizeBlock(stageBlock, scenarioType, widgetSetID, textureKitID)
	--ScenarioStage_UpdateOptionWidgetRegistration(stageBlock, widgetSetID);  -- MSA
...
function SCENARIO_CONTENT_TRACKER_MODULE:Update()
	local scenarioName, currentStage, numStages, flags, hasBonusStep, isBonusStepComplete, _, xp, money, scenarioType, _, textureKitID = C_Scenario.GetInfo();  -- MSA
	local rewardsFrame = ObjectiveTrackerScenarioRewardsFrame;
	if ( numStages == 0 ) then
		ScenarioBlocksFrame_Hide();
		return;
	...
	if ( not ScenarioProvingGroundsBlock.timerID and not scenariocompleted ) then
		if (weightedProgress) then
			...
			self:UpdateCriteria(numCriteria, objectiveBlock, hasBonusStep, isBonusStepComplete);  -- MSA
	...
	--ObjectiveTracker_AddBlock(ScenarioWidgetContainerBlock);  -- MSA
...
function SCENARIO_CONTENT_TRACKER_MODULE:UpdateCriteria(numCriteria, objectiveBlock, hasBonusStep, isBonusStepComplete)
	if not self:ShouldShowCriteria() then
		return;
	end
	...
	for criteriaIndex = 1, numCriteria do
		...
		if criteriaInfo then
			...
			if criteriaInfo.duration > 0 and criteriaInfo.elapsed <= criteriaInfo.duration then
				SCENARIO_TRACKER_MODULE:AddTimerBar(objectiveBlock, objectiveBlock.currentLine, criteriaInfo.duration, GetTime() - criteriaInfo.elapsed);
	...
	-- MSA - bonus objectives
	local bonusHeader = ScenarioBonusHeader
	if hasBonusStep then
		local _, _, numBonusCriteria, bonusStepFailed = C_Scenario.GetBonusStepInfo()
		-- text and glow
		...
		local anchor = objectiveBlock.currentLine
		if anchor then
			bonusHeader:SetPoint("TOPLEFT", anchor, "BOTTOMLEFT", 0, -objectiveBlock.module.lineSpacing)
		else
			bonusHeader:SetPoint("TOPLEFT", 0, -objectiveBlock.module.lineSpacing)
		end
		objectiveBlock.height = objectiveBlock.height + bonusHeader:GetHeight() + objectiveBlock.module.lineSpacing
		objectiveBlock.currentLine = bonusHeader
		bonusHeader:Show()

		for i = 1, numBonusCriteria do
			local bonusIndex = numCriteria + i
			...
			if criteriaCompleted then
				local existingLine = objectiveBlock.lines[bonusIndex]
				local colorStyle = not bonusStepFailed and OBJECTIVE_TRACKER_COLOR["Complete"] or OBJECTIVE_TRACKER_COLOR["Normal"]
				line = SCENARIO_TRACKER_MODULE:AddObjective(objectiveBlock, bonusIndex, criteriaString, nil, nil, OBJECTIVE_DASH_STYLE_SHOW, colorStyle)
				line.Icon:Show()
				...
				local iconType = criteriaFailed and "Objective-Fail" or "Objective-Nub"
				local colorStyle = criteriaFailed and OBJECTIVE_TRACKER_COLOR["Failed"] or OBJECTIVE_TRACKER_COLOR["Normal"]
				line = SCENARIO_TRACKER_MODULE:AddObjective(objectiveBlock, bonusIndex, criteriaString, nil, nil, OBJECTIVE_DASH_STYLE_HIDE, colorStyle)
				line.Icon:Show()
				line.Icon:SetAtlas(iconType, true)
			end
			-- darken criteria lines if bonus step has failed
			if bonusStepFailed and not criteriaFailed then
				local colorStyle = OBJECTIVE_TRACKER_COLOR["Disable"]
			...
			if timeLeft and timeLeft > 0 and not criteriaCompleted and not criteriaFailed then
				SCENARIO_TRACKER_MODULE.lineSpacing = 4
				SCENARIO_TRACKER_MODULE:AddTimerBar(objectiveBlock, line, timeLeft, GetTime())
			elseif line.TimerBar then
				SCENARIO_TRACKER_MODULE:FreeTimerBar(objectiveBlock, line)
			end
		end
	else
		bonusHeader:Hide()
...
-- MSA
function ScenarioBonusHeader_OnEnter(self)
	...
	GameTooltip:SetOwner(self, "ANCHOR_NONE")
	GameTooltip:SetPoint("TOPRIGHT", self, "TOPLEFT",  0, -1)
