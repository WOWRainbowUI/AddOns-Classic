...
function GoToCCSettings(msg, editbox)
	if msg == "" or msg == nil then
		...
		Settings.OpenToCategory(L.Options[localeString]["ClassicCalendar"] or AddonTitle)
	end
end
...
CCIOFrame.name = L.Options[localeString]["ClassicCalendar"] or AddonTitle -- 選單名稱
...
local function createHorizontalRule(text, anchorFrame)
	...
	hrLineText:SetFont(STANDARD_TEXT_FONT, 14)
...
lblTitle:SetFont(STANDARD_TEXT_FONT, 16)
lblTitle:SetPoint("TOPLEFT", CCIOFrame, "TOPLEFT", 12, -12)
lblTitle:SetText("|cFFEFC502" .. (L.Options[localeString]["ClassicCalendar"] or AddonTitle) .. " (v" .. C_AddOns.GetAddOnMetadata(AddonName, "Version") .. ")|r")
...
local category = Settings.RegisterCanvasLayoutCategory(CCIOFrame, L.Options[localeString]["ClassicCalendar"] or AddonTitle)
category.ID = L.Options[localeString]["ClassicCalendar"] or AddonTitle
