...
local function createBlizzOptions()
	...
	local blizzPanel = dialog:AddToBlizOptions("TomTom-General", options.args.general.name, L["TomTom"])

	-- Coordinate Block Options
	config:RegisterOptionsTable("TomTom-CoordBlock", options.args.coordblock)
	dialog:AddToBlizOptions("TomTom-CoordBlock", options.args.coordblock.name, L["TomTom"])

	-- Crazy Taxi Options
	config:RegisterOptionsTable("TomTom-CrazyTaxi", options.args.crazytaxi)
	dialog:AddToBlizOptions("TomTom-CrazyTaxi", options.args.crazytaxi.name, L["TomTom"])

	-- Minimap Options
	config:RegisterOptionsTable("TomTom-Minimap", options.args.minimap)
	dialog:AddToBlizOptions("TomTom-Minimap", options.args.minimap.name, L["TomTom"])

	-- World Map Options
	config:RegisterOptionsTable("TomTom-Worldmap", options.args.worldmap)
	dialog:AddToBlizOptions("TomTom-Worldmap", options.args.worldmap.name, L["TomTom"])

	-- World Map Options
	config:RegisterOptionsTable("TomTom-Feeds", options.args.feeds)
	dialog:AddToBlizOptions("TomTom-Feeds", options.args.feeds.name, L["TomTom"])

	-- POI Options
	if TomTom.WOW_MAINLINE then
		config:RegisterOptionsTable("TomTom-POI", options.args.poi)
		dialog:AddToBlizOptions("TomTom-POI", options.args.poi.name, L["TomTom"])
	end
	...
	dialog:AddToBlizOptions("TomTom-Profiles-Waypoints", w_options.name, L["TomTom"])
	dialog:AddToBlizOptions("TomTom-Profiles-Options", p_options.name, L["TomTom"])
	return blizzPanel
end
...
function addon:CreateConfigPanels()
	...
	local aboutFrame = dialog:AddToBlizOptions("TomTom", L["TomTom"])
...
SlashCmdList["TOMTOM"] = function(msg)
	...
	if Settings then
		Settings.OpenToCategory(L["TomTom"])
	elseif InterfaceOptionsFrame_OpenToCategory then
		InterfaceOptionsFrame_OpenToCategory(L["TomTom"])
		InterfaceOptionsFrame_OpenToCategory(L["TomTom"])
	end
end
