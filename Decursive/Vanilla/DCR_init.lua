...
local function InitVariables_Once() -- {{{
    ...
    D.Revision = "422dc88"; -- not used here but some other add-on may request it from outside
    D.date = "2025-10-25T22:02:03Z";
    D.version = "2.7.31";

    if D.date ~= "@project".."-date-iso@" then
        -- 1761429723 doesn't work
...
function D:VersionWarnings(forceDisplay) -- {{{
    ...
    if DcrMaxTOC < T._tocversion then
        ...
        elseif time() - self.db.global.TocExpiredDetection > 3600 * 24 * 40 or debug then -- if more than 40 days elapsed since the detection
            ...
            if time() - self.db.global.LastExpirationAlert > 48 * 3600 or forceDisplay or debug then
                ...
                T._ShowNotice ("|cff00ff00Decursive version: 2.7.31|r\n\n" .. "|cFFFFAA66" .. L["TOC_VERSION_EXPIRED"] .. "|r");
    ...
    if (("2.7.31"):lower()):find("beta") or ("2.7.31"):find("RC") or ("2.7.31"):find("Candidate") or alpha then
        ...
        if self.db.global.NonRelease ~= "2.7.31" then
            self.db.global.NonRelease = "2.7.31";
...
T._LoadedFiles["DCR_init.lua"] = "2.7.31";
...
422dc88f403487b192d23564d3cf325a7ad43dcd
